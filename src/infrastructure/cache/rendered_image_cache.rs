...
use crate::{application::{interface::rendered_image_cache::{cache::RenderedImageCache, error::RenderedCacheError}, types::rendered_image::RenderedImage}, domain::value_object::image_id::image_id::ImageId};
...
impl RenderedImageCache for RenderedImageCacheInMemory {
    fn save(&self, image: RenderedImage) {
        let mut caches = self.caches.lock().expect("RenderedImageCache Mutex is Poisoned");
