...
mod undo_redo_usecase_test {
    ...
    use crate::{
        ...
        domain::{
            entity::{image::image::Image, session::session::Session},
            repository::{
    ...
    fn _set_up() -> (
        ...
        let upload_usecase = UploadUseCase::new(
            session_repo.clone(),
            image_repo.clone(),
            loader,
            segmenter.clone(),
        ...
        let segment_usecase = SegmentUseCase::new(
            session_repo.clone(),
            image_repo.clone(),
            segmenter.clone(),
            editing_session_repo.clone(),
        ...
        let segment_input = SegmentInput::new(
            session_id,
            Point::new(Coordinate::new(30, 40), PointLabel::BACKGROUND),
        );
    ...
    fn test_undo_execute_returns_segmented_image() {
        let (session_repo, image_repo, segmenter, editing_session_repo, image_cache, session_id) =
            _set_up();
        let usecase = UndoUseCase::new(
            session_repo.clone(),
            image_repo.clone(),
            segmenter.clone(),
            editing_session_repo.clone(),
    ...
    fn test_undo_redo_execute_returns_segmented_image() {
        let (session_repo, image_repo, segmenter, editing_session_repo, image_cache, session_id) =
            _set_up();
        let undo_usecase = UndoUseCase::new(
            session_repo.clone(),
            image_repo.clone(),
            segmenter.clone(),
            editing_session_repo.clone(),
        ...
        let redo_usecase = RedoUseCase::new(
            session_repo.clone(),
            image_repo.clone(),
            segmenter.clone(),
            editing_session_repo.clone(),
